...
public class Graph {
	...
	private boolean findPathBetweenNodes(Node node, String secondNode) {
		if(node.getName().equals(secondNode)) {
			return true;
		}
		hashVisitedNodes.put(node.getName(), true);
		for(Node n: node.getNeighbors()) {
			if(!hashVisitedNodes.containsKey(n.getName()))
					return findPathBetweenNodes(n, secondNode);
